...
const db = ICD10_MASTER_DB.map((old, i) => {
    ...
    return {
        ...
        common_aliases: enriched ? [...(enriched.exactMatches || []), ...(enriched.synonyms || []), ...(enriched.commonMisspellings || [])].slice(0, 5) : [],
...
const textToAppend = `
// ═══════════════════════════════════════════════════════════════════════════
// NEW PROMPT DATABASE (For Gemini Integration)
// ═══════════════════════════════════════════════════════════════════════════

export interface PromptICD10Condition {
  id: string;
  specialty: string;
...
export const ICD10_DATABASE: PromptICD10Condition[] = ${JSON.stringify(db, null, 2)};

export function getAllMatchTerms(condition: PromptICD10Condition): string[] {
  return [
...
export function getConditionByCodePrompt(code: string): PromptICD10Condition | undefined {
...
export function getR69Fallback(): PromptICD10Condition {
  return ICD10_DATABASE.find(c => c.id === "FLOOR-001") as PromptICD10Condition;
}
`;

fs.appendFileSync('data/icd10MasterDatabase.ts', textToAppend);
console.log('Appended prompt database functions successfully.');
