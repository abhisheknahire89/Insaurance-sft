...
import { ICDLookupResult } from '../../types/icd.types';

interface ICDSuggestionDisplayProps {
  result: ICDLookupResult;
    ...
    <div className="border border-white/20 rounded-lg p-4 bg-gray-900 shadow-md">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-2xl font-mono font-bold text-blue-400">{result.code}</span>
          <span className="text-gray-300 font-medium">{result.description}</span>
        </div>
      ...
      <div className="mt-3 text-xs text-gray-500 bg-gray-800/50 p-2 rounded flex items-center gap-2">
        <span className="font-medium text-gray-400">Match tier:</span> 
        <span className="text-blue-300">Tier {result.tier}</span>
      </div>
      ...
      {result.tier !== 'FLOOR' && result.confidence > 0 && result.confidence < 70 && (
      ...
      {result.confidence >= 70 && result.tier !== 'FLOOR' && (
      ...
      {result.condition_data && result.condition_data.icd_codes.variants && result.condition_data.icd_codes.variants.length > 0 && (
        <div className="mt-4 pt-3 border-t border-gray-800">
          <p className="text-xs text-gray-400 mb-2 font-medium">Other possible codes:</p>
          <div className="flex gap-2 flex-wrap">
            {result.condition_data.icd_codes.variants.map(variant => (
              <button 
                key={variant.code}
                ...
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSelectAlternative(variant.code); }}
